...
This file is not shared here as it might contain sensitive information and needs to be requested from one of the leads of the CCHIC team. If the file cannot be provided for any reason, it is relatively easy to writing for the CCHIC dataset by following the template in `data/texas.json`. 
- Request from the CCHIC leads the following extra files:
  - `hash.json`: This is a dictionary hash that converts patient ID labels to the letter IDs shown in the report. This should be placed in the root directory of this repo. This file can easily be generated if not available by writing a simple .json containing a dictionary with the mappings. If the hash is not used, the plots will contain the actual patient IDs.
  - `tests/inference/runconfig_cchic.json`; `tests/linkage/runconfig_cchic.json`; `tests/utility/runconfig_cchic.json`: These files are in the repo but they are missing the target IDs used when running the report's experiments. The files used for attribute inference is also missing the real prior values for HIV and CIR prevalence. The CCHIC leads will be able to provide you with the versions used which contain the real target IDs and prior values. These should replace the `.json` files under the `tests/` directory. If the sensitive files are not available, the user can choose average and outlier patient targets by looking at the CCHIC data and put their IDs in the runconfig files.
